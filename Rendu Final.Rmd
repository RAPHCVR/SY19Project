...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE, 
                      fig.align = 'center', fig.pos = 'H', fig.height = 3)
...
```{r simu_reg_eda}
# Dimensions et résumé
cat(sprintf("Régression: n=%d, p=%d\n", nrow(df_reg), ncol(df_reg)-1))
summary(df_reg$y)

# Valeurs manquantes
na_reg <- colSums(is.na(df_reg))
na_reg[na_reg > 0]

# Distribution de y (compact)
ggplot(df_reg, aes(x = y)) + 
  geom_histogram(fill = "steelblue", bins = 30) + 
  labs(title = "Distribution de y (régression)")
```

### Classification — vue d’ensemble

```{r simu_cla_eda}
# Dimensions et balance des classes
cat(sprintf("Classification: n=%d, p=%d\n", nrow(df_cla), ncol(df_cla)-1))
table(df_cla$y)
prop.table(table(df_cla$y))

# Valeurs manquantes
na_cla <- colSums(is.na(df_cla))
na_cla[na_cla > 0]

# PCA colorée par classe
Xc <- df_cla[, -which(names(df_cla)=="y")]
pc_c <- prcomp(scale(Xc), center = TRUE, scale. = TRUE)
pc_df <- data.frame(pc_c$x[,1:2], y = factor(df_cla$y))
ggplot(pc_df, aes(PC1, PC2, color = y)) + 
  geom_point(alpha = 0.6) + 
  labs(title = "PCA (2 premières composantes) - classes")
```

## Régression (Minimisation de la MSE)
...
### Diagnostics du modèle de régression

```{r simu_reg_diag}
y_hat <- as.numeric(predict(final_model_reg, newx = X_reg))
res <- y_reg - y_hat
par(mfrow = c(1,2))
plot(y_hat, res, pch = 19, col = "darkgray",
  main = "Résidus vs prédictions", xlab = "ŷ", ylab = "résidus")
qqnorm(res); qqline(res, col = "red")
cat(sprintf("MSE (apprentissage): %.4f\n", mean(res^2)))
```

## Classification (Maximisation du taux de bon classement)
...
### Évaluation du classifieur (matrice de confusion, ROC)

```{r simu_cla_eval, message=FALSE}
library(pROC)
pred <- predict(final_model_cla, newdata = X_cla)
pred_class <- pred$class
conf <- table(Vérité = y_cla, Prédiction = pred_class)
conf
acc <- sum(diag(conf))/sum(conf)
cat(sprintf("Exactitude (apprentissage): %.3f\n", acc))

if (!is.null(pred$posterior) && ncol(pred$posterior) == 2) {
  pos_level <- levels(y_cla)[2]
  roc_obj <- roc(response = y_cla, predictor = pred$posterior[, pos_level])
  cat(sprintf("AUC (apprentissage): %.3f\n", auc(roc_obj)))
}
```

## Formatage pour la soumission (Fichier .Rdata)
...
# Données Réelles : Seoul Bike Sharing Demand
...
## Modélisation : Gradient Boosting (XGBoost)
